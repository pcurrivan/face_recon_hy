...
cur = conn.cursor()
# GET METADATA
cur.execute("SELECT (num_frames,frame_width,frame_height) FROM video_metadata WHERE video_id={v};".format(v=videoId))
data = cur.fetchone()
data = data[0]
data = data.replace("(", "")
data = data.replace(")", "")
print(data)
numFrames, frameWidth, frameHeight = data.split(",")
